...
// const md5 = require('md5') 懒得刷user 库 用回原来的（不加算法 随机数）
const md5 = val => val
...
app.route('/register')
  ...
  .post((req, res, next) => {
    ...
    if (!USERNAME_RE.test(regInfo.name)) {
      ...
      var result = addUser.run(regInfo.name, md5(regInfo.password), regInfo.email, regInfo.avatar || 'http://localhost:8006/uploads/avatar.png')
      console.log(result)
      res.redirect('/login')
    }
  })
...
app.route('/login')
  ...
  .post((req, res, next) => {
    ...
    var user = userStmt.get(regInfo.name, md5(regInfo.password))
...
app.post('/forget-password', (req, res, next) => {
  ...
  var link = 'http://10.0.0.167:8006/forget-password/' + id
...
app.get('/forget-password/:id', (req, res, next) => {
  var email = changePassMap[req.params.id] //id 对应的邮箱是否存在 
  if (!email) {
...
app.delete('/comment/:id', (req, res, next) => {
  ...
  //此处有错误，应该拿评论所属用户的id来对比，而req.params.id是评论自己的id
